...
.PHONY: all build test vet fmt check demo cores admission-front opsec golden clean unguarded dist dist-tar tar-only sign notarize release publish publish-check locale-demo
...
#
# The source is KingKlaw's admission member (MIT), kept out of this AGPL tree
# on purpose so that anyone can take the gate without taking ghillie. Point
# KINGKLAW at your checkout; the build needs the free FSF GNAT + GNATprove
