...
Prebuilt binaries for Windows, macOS and Linux are on the
[releases page](https://github.com/the-dark-factory/Ghillie_AGPL/releases/latest),
with SHA-256 checksums alongside them. The macOS builds are signed and notarised.

**If you just want to run it, read [README-QUICKSTART.md](README-QUICKSTART.md).**
It verifies the download, unpacks it and gets you to a first result, with separate
instructions for each platform. The rest of this file is about how the thing is
built and why it can be trusted — useful, but not the way in.
...
Built to the ghillie facade protocol §2–§6 and the v1 interview-loop plan.
Nothing in this repository redesigns any part of it.
...
### PROVEN — Ada/SPARK cores forged in the factory, admitted, captured
...
from the factory's own working tree (read-only — nothing there is touched),
...
built — **spoken through the settled breath pipeline** (the
...
Everything lands under a timestamped demo-run directory (never
